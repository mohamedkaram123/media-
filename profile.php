...
$stmt = $con->prepare("SELECT cover FROM users Where Username = ?");
    ...
    $pathomguploads = "uploads/covers/" . $rowc["cover"];
 ...
 echo '<div class="page-header header-filter" data-src='.$pathomguploads.' data-parallax="true" style="background-image:url('.$pathomguploads.');">';
    ...
    <input type="file" id="file3" class="covering" name="cover" />  
    </div>
</div>
    ...
    $pathomguploads = "uploads/avatars/" . $row["avatar"];
                    ...
                    echo "<img src=".$pathomguploads." id='imge' class='img'  data-delete=".$pathomguploads.">";
                         ...
                         <div id="popo">
                            ...
                            <span class="name-prof"><?php echo $_SESSION['user']; ?></span>
...
$stmt = $con->prepare("SELECT Post_ID FROM actions where actions.UserID = ? ");
    $stmt->execute(array($roww["UserID"]));
    $rows =$stmt->fetchAll(); 
...
$stmt->execute(array($roww["UserID"],$roww["UserID"]));
$rows =$stmt->fetchAll();
?>
...
/*====================================================================================================================== */
    ...
    $("#popo").addClass("popo");
    $("#i").addClass("fas fa-camera iconcamera2");
})
...
$("#i").removeClass("fas fa-camera iconcamera2");
})
...
$(document).on("change","#file3",function(){
       ...
       let files = new FormData();
     files.append('file3', $('#file3')[0].files[0]);
...
type: "GET",
url: "avatarajaxprofile.php",
data: "save_cover="+$("#img_cover").attr('data-src'),
success:function(data) {
//   $(".box-border").css("height","395px");
...
$(".delete-cover").remove();
   ...
   $(".delete-cover").click(function(){
                ...
                type: "GET",
                url: "avatarajaxprofile.php",
                data: "delete_cover="+$("#img_cover").attr('data-deletecover'),
                success:function(data) {
